...
def tool_to_action(tool_use_name: str,tool_use_params: str):
    tool_use_params = json.loads(tool_use_params)
    if tool_use_name == "view_directory":
        return f"view the directory of {tool_use_params['dir_path']}"
    elif tool_use_name == 'search_files_by_keywords':
        if 'directory' in tool_use_params:
            return f"search the files under directory {tool_use_params['directory']} with keywords {tool_use_params['keywords']}"
        else:
            return f"search the files under project root directory with keywords {tool_use_params['keywords']}"
    elif tool_use_name == 'view_file_content':
        return f"view the whole content of file {tool_use_params['file_name']}"
    elif tool_use_name == 'view_file_structure':
        return f"view the structure of file {tool_use_params['file_path']}"
    elif tool_use_name == 'str_replace_editor':
        return f"use the str_replace_editor tool with params: {tool_use_params}"
    else:
        print(f"Warning: unknown tool use name: {tool_use_name}")

def extract_observation(xml_string: str) -> str:
    match = re.search(r'<observation>(.*?)<\/observation>', xml_string, re.DOTALL)
    return match.group(1).strip() if match else None

def compress_agent_action_deepseek(messages: list,agent_name: str): 
    """
    Compress the agent action into a single message (specific for deepseek)
    """
    if len(messages) == 1:
        print(f"Warning: no action found for {agent_name}")
        return messages

    if "problem solver" in agent_name.lower():
        agent_type = "problem solver"
        default_action = f"{agent_type} starts to solve the problem:"
    elif "problem decoder" in agent_name.lower():
        agent_type = "problem decoder"
        default_action = f"{agent_type} starts to decode the problem:"
    elif "solution mapper" in agent_name.lower():
        ...
        try:
            if isinstance(msg, AIMessage) and  i != len(messages) - 1:
                ...
                if content and content != "":
                    observation = extract_observation(content)
                    if observation:
                        action_list.append(f"{agent_type}'s observation on tool call result:\n{observation}\n")
                
                tool_content = msg.tool_calls[0]
                
                tool_use_name = tool_content.get('name', 'unknown_tool')
                tool_use_params = json.dumps(tool_content.get('args'))
                if tool_use_params:
                    action_description = tool_to_action(tool_use_name, tool_use_params)
                    action_list.append(f"action {action_step}: {agent_type} {action_description}")
                else:
                    action_list.append(f"action {action_step}: {agent_type} uses tool: {tool_use_name}")
            
                action_step += 1
        
            elif i == len(messages) - 1:
                agent_action = "\n".join(action_list).rstrip()
                compressed_message = AIMessage(content=agent_action, name=agent_name)
            ...
            return None
    print("error: not found last message")
    return None


def compress_agent_action(messages: List, agent_name: str) -> Optional[List]:
    """
    Compress the agent actions into a single, structured message.
    ...
    Returns:
        Optional[List]: A compressed list containing the summary message and the final message,
                        or None if an error occurs.
    """
    # model_message = messages[-1].response_metadata['model_name']
    model_name = 'deepseek'
    if "deepseek" in model_name.lower():
        return compress_agent_action_deepseek(messages, agent_name)
        ...
        try:
            ...
            if isinstance(msg, AIMessage) and i != len(messages) - 1:
                ...
                if isinstance(content, list) and content:
                    # Handle messages involving tool calls
                    observation = None
                    tool_content = None

                    if len(content) > 1:
                        observation = extract_observation(content[0].get('text', ''))
                        tool_content = content[1]
                    else:
                        tool_content = content[0]
                    ...
                    tool_use_name = tool_content.get('name', 'unknown_tool')
                    tool_use_params = (
                        json.dumps(tool_content.get('input'))
                        if tool_content.get('input') is not None
                        else str(tool_content.get('partial_json'))
                        if tool_content.get('partial_json') is not None
                        else None
                    )
            ...
            # If last message, finalize compression
            elif i == len(messages) - 1:
                agent_action = "\n".join(action_list).rstrip()
                compressed_message = AIMessage(content=agent_action, name=agent_name)
                return [compressed_message, msg]
    ...
    print("Error: Final message not found or processed.")
    return None